...
#define LOBJECT_DEFINE_CLASS(CLASSNAME, TYPENAME, LUANAME) CLASSNAME(lutok2::state & state) : LObject<CLASSNAME, TYPENAME>::LObject( state, (LUANAME))
#define LOBJECT_INSTANCE(CLASSNAME) (CLASSNAME::getInstance(state))
#define LOBJECT_METHOD(METHODNAME, TYPEDEF) METHODNAME(lutok2::state & state, TYPEDEF)
#define LOBJECT_OPERATOR(OPERATORNAME, TYPEDEF) operator_##OPERATORNAME(lutok2::state & state, TYPEDEF)
...
public:
	virtual ~LSingleton() {};
	static inline C & getInstance(lutok2::state & state);
private:
...
public:
	...
	typedef int (C::*Method) (lutok2::state &, T);
	...
	int null_method(lutok2::state &, T){
		return 0;
	}
...
protected:
	...
	LObject(lutok2::state & state, const std::string & className){
		this->state = state;
...
/*
  ...
  Arguments:
    * L - Lua State
*/
    static int constructor(lutok2::state & s){
		return 0;
    }

	void destructor(lutok2::state & s, T obj){
    }
...
/*
  ...
  Arguments:
    * L - Lua State
*/
    static int property_getter(lutok2::state & s){
		s.get_metatable(1);
		s.push_value(2);
		s.raw_get();
...
/*
  ...
  Arguments:
    * L - Lua State
*/
    static int property_setter(lutok2::state & s){
...
/*
  ...
  Arguments:
    * L - Lua State
*/
	static int operator_global(lutok2::state & s, const char * name){
		s.get_metatable(1);
		s.push_string(name);
		s.raw_get();
		if (s.is_number()) {
...
/*
  ...
  Arguments:
    * L - Lua State
*/
    static int function_dispatch(lutok2::state & s) {
...
/*
  ...
  Arguments:
    * L - Lua State
*/
    static int gc_obj(lutok2::state & s)	{
	...
	static int to_string(lutok2::state & s) {
		LObjectTuple ** obj = s.to_userdata<LObjectTuple *>();
		C * thisobj = std::get<0>(**obj);
